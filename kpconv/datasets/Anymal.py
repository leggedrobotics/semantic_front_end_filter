...
class AnymalDataset(PointCloudDataset):
    ...
    def __init__(self, config, train=True, orient_correction=True):
        ...
        """
        PointCloudDataset.__init__(self, 'Aynmal')
    ...
    def __getitem__(self, idx_list):
        ...
        #show_ModelNet_examples(tp_list, cloud_normals=tf_list)
        ...
        stacked_features = np.hstack([
            np.ones_like(stacked_points[:, :1], dtype=np.float32), # vector of ones
            stacked_features])
    ...
    def load_subsampled_clouds(self, orient_correction):
        ...
        # Else compute them from original points
            ...
            if self.train:
                ...
                expnames = glob(join(self.path, '*'))
                names = [n for en in expnames for n in glob(join(self.path, en, '*.msgpack'))][:self.num_models]
            else:
                expnames = glob(join(self.path, '*'))
                names = [n for en in expnames for n in glob(join(self.path, en, '*.msgpack'))][-self.num_models:]
            ...
            # Get labels
            input_labels = np.array([0 for p in input_points])
...
class AnymalSampler(Sampler):
    """Sampler for Anymal dataset"""

    def __init__(self, dataset: AnymalDataset, use_potential=True, balance_labels=False):
