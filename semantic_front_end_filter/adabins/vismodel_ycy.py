...
def vis_one(loader="test", figname=""):
    ...
    for i, (model, name) in enumerate(zip(model_list, names_list)):
        ...
        pred = nn.functional.interpolate(torch.tensor(
            pred)[None, ...], torch.tensor(depth).shape[-2:])
        pred = pred[0][0].numpy()
        ...
        axs[plot_ind//4, plot_ind %
            4].imshow(mask_weight.detach().cpu().numpy()[0, 0], vmin=0, vmax=1, alpha=0.5)
        axs[plot_ind//4, plot_ind %
            4].imshow(cv2.cvtColor(inputimg[:, :, :3], cv2.COLOR_BGR2RGB), alpha=0.5)
        ...
        plt.colorbar(
            cax=cax, mappable=axs[plot_ind//4, plot_ind % 4].images[0])
        ...
        axs[plot_ind//4, plot_ind %
            4].imshow(sample["mask_gt"].detach().cpu().numpy()[0, 0])
        axs[plot_ind//4, plot_ind %
            4].imshow(cv2.cvtColor(inputimg[:, :, :3], cv2.COLOR_BGR2RGB), alpha=0.5)
        ...
        axs[plot_ind//4, plot_ind % 4].plot(pc_img[mask_pc].reshape(-1),
                                            pcdiff[mask_pc].reshape(-1), "x", ms=1, alpha=0.2, label="pc_img_err")
        axs[plot_ind//4, plot_ind % 4].plot(depth[mask_traj].reshape(-1),
                                            diff[mask_traj].reshape(-1), "x", ms=1, alpha=0.2, label="traj_err")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--models", default="/home/anqiao/tmp/semantic_front_end_filter/checkpoints/2023-01-05-23-53-59_4Grass&Forest/UnetAdaptiveBins_latest.pt")
    parser.add_argument("--names", default="")
    parser.add_argument(
        "--outdir", default="/home/anqiao/tmp/semantic_front_end_filter/checkpoints/2023-01-05-23-53-59_4Grass&Forest/results_latest_test")
