...
def main_worker(gpu, ngpus_per_node, args):
    ...
    args.epoch = 0
    args.last_epoch = -1
    return train(model, args, epochs=args.trainconfig.epochs, lr=args.trainconfig.lr, device=args.gpu, root=args.root,
...
def objective(trial):
    args.trainconfig.lr = trial.suggest_float("lr", 1e-5, 3e-3)
    args.trainconfig.traj_label_W = trial.suggest_float("trajW", 1, 6)
...
if __name__ == '__main__':
    ...
    if args.distributed:
        ...
        print("  Values: ", trials.values)

        print("  Params: ")
        for key, value in trials.params.items():
            print("    {}: {}".format(key, value))
        fig = optuna.visualization.plot_pareto_front(study, target_names=["traj_rmse", "pc_rmse"])
        fig.write_html("/cluster/home/anqiali/semantic_front_end_filter/pc=larger_pareto-front.html")
        # fig = optuna.visualization.plot_param_importances(
        #     study, target=lambda t: t.values[0], target_name="traj_rmse"
        # )
        # fig.write_html("/cluster/home/anqiali/semantic_front_end_filter/importance_to_traj_rmse.html")
        # fig = optuna.visualization.plot_param_importances(
        #     study, target=lambda t: t.values[0], target_name="pc_rmse"
        # )
        # fig.write_html("/cluster/home/anqiali/semantic_front_end_filter/importance_to_pc_rmse.html")
