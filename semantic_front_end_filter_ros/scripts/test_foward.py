...
def callback(image, point_cloud):
    ...
    with Timer("Update"):
        ...
        # pointclouds
        try:
            listener.waitForTransform("map", point_cloud.header.frame_id, rospy.Time(0), rospy.Duration(1.0))
            (trans,rot) = listener.lookupTransform("map", point_cloud.header.frame_id, rospy.Time(0))
        ...
        pc_array = np.copy( np.frombuffer(surf.tobytes(), np.dtype(np.float32) ).reshape(surf.shape[0],-1)[:,:3] )
