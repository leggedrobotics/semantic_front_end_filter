...
GridMap getMap(std::string map_file_path){ 
  // Get data from file
  ...
  for (GridMapIterator it(map); !it.isPastEnd(); ++it) {
    ...
    if(grid_x >=0 && grid_x < gm.GroundArray.size() && grid_y >=0 && grid_y < gm.GroundArray[0].size()){
      if (abs(gm.GPMap[grid_x][grid_y])>0.000000000001){
...
{ 
  ...
  int map_num;
  nh.getParam("maps_path", maps_path);
  nh.getParam("map_num", map_num);
  ...
  for(int i=0; i<map_num; i++){
    char index_string[8];
    snprintf (index_string, 8, "%03d", i);
    std::cout<<"index: "<<index_string<<std::endl;
    std::string map_file_path = maps_path + "/localGroundMap_" + index_string + ".msgpack";  
    GridMap map = getMap(map_file_path);
    maps.push_back(map);
    
    std::string map_topic = "grid_map"+std::string(index_string);
    ros::Publisher publisher = nh.advertise<grid_map_msgs::GridMap>(map_topic, 1, true);
  ...
  while (nh.ok()) {
    ...
    for(int i=0; i<map_num; i++){
      maps[i].setTimestamp(time.toNSec());
      grid_map_msgs::GridMap message;
      GridMapRosConverter::toMessage(maps[i], message);
    ...
    ROS_INFO_THROTTLE(1.0, "Grid maps published.");
